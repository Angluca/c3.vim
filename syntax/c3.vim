...
syn keyword PreProc extern inline static tlocal
syn keyword Keyword var const
...
hi def link c3ComptimeId   Label
...
"hi def link c3Typedef Identifier
hi def link c3Typedef SpecialComment
hi def c3Type ctermfg=DarkCyan guifg=DarkCyan
"hi def c3This ctermfg=DarkMagenta guifg=DarkMagenta

syn match c3Typedef  contains=c3Typedef "\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*" display contained
"syn keyword Keyword   def nextgroup=Function skipwhite skipempty
syn keyword c3Keyword union struct enum namespace def fn macro const nextgroup=c3Typedef skipwhite skipempty

syn match Repeat      "\([^\.]\.\)\@<=\w\w*\(\(\[.*\]\)*\_s*(\)\@!"
syn match c3Function  "[0-9a-zA-Z_@]*\w\w*\(\(\[.*\]\)*\_s*(\)\@="
syn match c3Type      '(\=\_s*\(\[.*\]\_s*\)*\(const\_s*\)\=\zs\w\w*\ze\(\[.*\]\)*\_s*\(\[.*\]\)*\_s*{'
syn match c3Type     "\w\(\w\)*\ze\(<.*>\s*\)*::[^<]"
